...
from storage import CasePaths, init_case_paths, read_draft, write_draft, list_cases
from wizard import (
    WizardStateEnum,
    initial_payload,
    apply_answer,
...
def _save_current(payload: Dict[str, Any]) -> None:
    ...
    state_str = (payload.get("wizard") or {}).get("state", WizardStateEnum.DRAFT.value)
...
def _render_case_sidebar() -> None:
    ...
    if last_err:
        st.sidebar.warning("Final validation failed. You can continue editing. Validate will pass once required fields are completed.")
...
def _render_question(q, payload: Dict[str, Any]) -> Any:
    ...
    if q.kind == "slider":
        v = current_value if current_value is not None else q.slider_min
        return st.slider(
            q.label,
...
def _render_current_page(payload: Dict[str, Any]) -> None:
    ...
    err = validate_answer_for_question(q, answer)
    if err:
        st.error(err)
    ...
    with col2:
        if st.button("Save"):
            if err:
                ...
                payload = apply_answer(payload, q.key, answer)
                _save_current(payload)
                st.session_state["active_payload"] = payload
                st.success("Saved.")
    ...
    with col3:
        if st.button("Next"):
            if err:
                st.error("Fix the validation error before continuing.")
            else:
                payload = apply_answer(payload, q.key, answer)
